...
interface LongTermBorrowingsNoteProps {
  data: LedgerRow[];
  noteNumber: string;
  reportingScale?: string;
}

export function LongTermBorrowingsNote({ data, noteNumber, reportingScale = 'rupees' }: LongTermBorrowingsNoteProps) {
  ...
  const borrowingsLedgers = useMemo(() => {
    return data.filter(row => {
      ...
      return (h4.includes('long') && (h4.includes('borrowing') || h4.includes('loan'))) ||
             ...
             ledgerName.includes('debenture') || ledgerName.includes('bond');
    });
  }, [data]);
  ...
  const categories = useMemo(() => {
    ...
    borrowingsLedgers.forEach(ledger => {
      ...
      if (name.includes('secured') || name.includes('mortgage')) {
        secured.push(ledger);
      } else {
    ...
    <Card className="p-2">
      <CardHeader className="pb-2 pt-2">
        <CardTitle className="text-sm font-semibold">
          Note {noteNumber}: Long-term borrowings
        </CardTitle>
      </CardHeader>
