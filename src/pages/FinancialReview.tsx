...
const buildStockDetailRow = (params: {
  ...
  differenceForDebitCredit?: number;
  ...
  const debitCreditBase = params.differenceForDebitCredit !== undefined ? params.differenceForDebitCredit : closingValue;
...
const buildStockDetailRows = (values: ManualInventoryValues): LedgerRow[] => {
  ...
  const rows = STOCK_DETAIL_CATEGORIES.map(category => {
    ...
    // Opening Balance = -opening (NEGATIVE)



    // Closing Balance = -closing (NEGATIVE)



    // Difference for Debit/Credit: (-opening) - (-closing) = closing - opening



    const difference = opening - closing;  // Positive when inventory decreases
    ...
    return buildStockDetailRow({
      ...
      differenceForDebitCredit: difference,
  ...
  const rawMaterialOpening = values.rawMaterials?.opening || 0;



  const rawMaterialClosing = values.rawMaterials?.closing || 0;
  ...
  const fgOpening = values.finishedGoods?.opening || 0;



  const fgClosing = values.finishedGoods?.closing || 0;



  const sitOpening = values.stockInTrade?.opening || 0;



  const sitClosing = values.stockInTrade?.closing || 0;
  ...
  // Change = Opening - Closing (positive when inventory decreases)



  const rawMatChange = rawMaterialOpening - rawMaterialClosing;



  const wipChange = wipOpening - wipClosing;



  const fgChange = fgOpening - fgClosing;



  const sitChange = sitOpening - sitClosing;
  ...
  // Raw Materials goes to Cost of Materials Consumed



  // Opening Balance = opening (NOT negated), Closing Balance = -change (negated)
  ...
  rows.push(
    ...
    buildStockDetailRow({
      ...
      ledgerName: 'Change in Inventories [Raw Materials]',
      ...
      h3: 'Change in Inventories - Raw Materials',
      ...
      differenceForDebitCredit: rawMatChange,
  ...
  // Others (WIP + FG + SIT) goes to Change in Inventories



  // Opening Balance = total opening (NOT negated), Closing Balance = -change (negated)



  const othersOpening = wipOpening + fgOpening + sitOpening;



  const othersChange = wipChange + fgChange + sitChange;
  ...
  rows.push(
    ...
    buildStockDetailRow({
      ...
      differenceForDebitCredit: othersChange,
...
export default function FinancialReview() {
  ...
  const profitForYearAmount = faceSummaryPL.totals.profitLoss ?? 0;
  ...
  const displayNoteRows = useMemo<DisplayNoteRow[]>(() => {
    ...
    const computedRows = addProfitRow(computedState.rows);
  ...
  }, [activeNote, autoFlatRows, noteLayout, noteEditMode, numberScale, computeRowState, noteTolerance, isPartnershipEntityType, isNonCompanyEntityType, noteStatementType, entityType, profitForYearAmount, isCompanyEntityType]);
